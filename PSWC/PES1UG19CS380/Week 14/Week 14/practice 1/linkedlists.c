...
{
	int ctr=1;
	list_t * ennode=stnode;
	while(ennode->nextptr!=NULL)
	{
		if(ennode->data==FindElem)
			break;
		else
			ctr++;
	...
	printf("6) Exit\n ENTER...\n");
		...
		switch(q)
		{
			...
			case 4: printf("\n Enter the element you want to find: ");
				...
				else printf("element is not in this list\n"); 
				break;
			case 5:printf("\n Enter the number you want to be divisor: ");
				scanf("%d",&num);
				printf("product of noded divisible by %d is %d ", num, product(toad, num)); break;
